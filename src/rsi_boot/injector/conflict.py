...
class ConflictDetector:
    ...
    def _recent_adoption(self, item_id: str, markers: List[str]) -> int:
    ...
    async def scan(self, project_id: str = "") -> Dict[str, int]:
        ...
        for mem in memories:
            ...
            for f in files:
                ...
                if conflict_type is None and self._is_stale(f):
                    adoption = self._recent_adoption(mem.id, [m for m in [mem.domain, *tags] if m])
        ...
        version_detected = await self._scan_version_families(project_id, existing, seen, now)
        detected += version_detected
        closed = self._close_resolved(files, existing)
    ...
    async def _scan_version_families(
    ...
    def _close_resolved(self, files: List[UserRuleFile], rows: List[Dict[str, Any]]) -> int:
    ...
    async def list_conflicts(
        ...
        *, bootstrap_run_id: Optional[str] = None,
        ...
        if bootstrap_run_id:
            ...
            rows = [r for r in rows if self._in_run(r, marker)]
    ...
    async def resolve(
        ...
        if ctype in ("version", "doc_code", "incoherent"):
            guidance, activate_ids = self._resolve_pair(row, resolution)
    ...
    def _resolve_pair(self, row: Dict[str, Any], resolution: str) -> tuple[str, List[str]]:
        ctype = row.get("type") or row.get("conflict_type")
        item = self._read_doc(row.get("item_id"))
    ...
    def _activate_doc(self, doc: Any) -> bool:
        if doc is None or doc.status != "pending_review":
            return False
        try:
