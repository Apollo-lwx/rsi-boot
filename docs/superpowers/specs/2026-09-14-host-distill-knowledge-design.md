...
| 触发 | 宿主必做 | 落盘 |
|------|----------|------|
| **修了用户可见的 bug**（成功或失败都算） | `teach_catch` → 如何产生 + 如何修复 → `teach_record` | teaching + gene；**立刻可召回**，首次 weight=2（建议） |
| 用户纠正做法 / 将改公共规则 | 同上 | `author=user` 时 weight=10（你已定性，不是建议） |
| 低置信但未形成 bug | 可记 | 同首次修 bug，weight=2 |
...
| 事件 | 权重 | 召回 |
|------|------|------|
| 该 `error_signature` **第一次** `teach_record`（agent 修 bug，非用户纠正） | `weight=2`，`hit_count=1` | **马上可见**，排在禁止项之后，按 weight 升序靠后；文案/技能当「建议规避」，不写进 alwaysApply mdc |
| 该条 id 曾出现在某次 `rsi_recall` 的 `retrieved` 里，之后再次因同类 bug `teach_record` | 同一 id **加 2**（封顶 10），`hit_count+1`，可改 `correct_fix`；**不新建**一条 | 仍马上可见，排序随 weight 前移 |
| 再次 record 但中间**从未被召回命中** | 只追加 attempts，**不加** weight | 避免「没人用到却刷分」 |
| `author=user`（你纠正） | 直接 10 | 你已拍板，不是建议 |

`hit_count >= 3` 或 `weight >= 8`：成为 `rsi_learn promote` 候选（pending 约定/禁止项），**不**自动进正式 mdc。晋升仍走 review。
...
- 修 bug 成功也 `teach_record`；强制 `wrong_action` + `correct_fix`；首次 weight=2 且下次召回可见；命中后再修 +2，同签名不新建。
